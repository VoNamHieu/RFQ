...
export function CompanyAnalytics({ company }) {
  ...
  // "Compare only" forces the comparison on and hides every stat / block that carries no
  // previous-period delta (snapshot, all-time and config metrics).
      ...
      <Card>
        <BlockStack gap="300">
          <InlineGrid columns={{ xs: 2, sm: 4 }} gap="400">
            ...
            {!compareOnly && <Stat label="Open quote value" value={money(openQuoteValue)} sub="Current snapshot · not affected by date range" help="Total value of this company's quotes that are still open. This is a current snapshot and is not affected by the date range." />}
          </InlineGrid>
          {changedSentence ? <Text as="p" variant="bodyMd">{changedSentence}</Text> : null}
        </BlockStack>
      </Card>

      {/* "Compare only": relationship state, attention signals and activity are current snapshots */}
      {!compareOnly && (
        <>
  ...
  const buying = (
    <BlockStack gap="400">
      ...
      {/* "Compare only": top products and basket behaviour carry no previous-period comparison */}
      {!compareOnly && (
        <>
  ...
  const quotes = (
    <BlockStack gap="400">
      <Card>
        <InlineGrid columns={{ xs: 2, sm: 4 }} gap="400">
          {!compareOnly && <Stat label="Open quote value" value={money(openQuoteValue)} sub="Current snapshot" help="Total value of this company's quotes that are still open. This is a current snapshot." />}
          {!compareOnly && <Stat label="Open quotes" value={String(openQuotes.length)} help="Number of quotes that are still open and have not yet been won or lost." />}
          ...
          {!compareOnly && <Stat label="First response time" value={fmtDur(responseMedian)} help="How long this company's quotes typically take to receive their first price or reply." />}
        </InlineGrid>
      </Card>
  ...
  const pricing = (
    <BlockStack gap="400">
      <Card>
        <InlineGrid columns={{ xs: 2, sm: 4 }} gap="400">
          {!compareOnly && <Stat label="B2B price vs Shopify" value={vsShopify == null ? '—' : signedPct(vsShopify)} sub="on B2B-priced lines" help="How much lower or higher this company's B2B prices are than its regular Shopify prices." />}
      ...
      {/* "Compare only": quote-vs-pricing, per-pricing, quantity and below-margin detail carry no previous-period comparison */}
      {!compareOnly && (
        <>
          {varRows.length ? (
